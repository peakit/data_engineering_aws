...
print("With lambda doing a named function call: ", log_of_songs_rdd.map(lambda s: convert_song_to_lowercase(s)).collect())

# Show the original input data is still mixed case
print("Original is intact: ", log_of_songs)

# Use lambda functions instead of named functions to do the same map operation
print("With anonymous lambda: ", log_of_songs_rdd.map(lambda s: s.lower()).collect())